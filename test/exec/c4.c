    ...
    if (tk == '\n') {
      if (src) {
        ...
        while (le < e) {
          ...
          //FIX: disable the value dump because it changed in different between Linux and Windows.
          //if (*le <= ADJ) printf(" %d\n", *++le); else printf("\n");
        }
      }
      ++line;
...
int main(int argc, char ** argv)
{
  ...
  i = Char;
  while (i <= While) { next(); id[Tk] = i++; } // add keywords to symbol table

  i = OPEN;
  while (i <= EXIT) { next(); id[Class] = Sys; id[Type] = INT; id[Val] = i++; } // add library to symbol table
