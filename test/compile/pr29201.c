...
MetaMoveResizeFlags;
int adjust_for_gravity (MetaWindow * window, MetaFrameGeometry * fgeom,
...
void 
meta_window_constrain (void*window, void*, MetaMoveResizeFlags flags,
             int resize_gravity, MetaRectangle *, MetaRectangle *);

void meta_topic_real (MetaStackLayer x, bool);
int meta_window_move_resize_internal (MetaWindow * window,
                  ...
                  int root_y_nw, int w, int h)
{
  ...
  {
    adjust_for_gravity (window, window->frame ? &fgeom : ((MetaFrameGeometry*) 0),
            is_configure_request, window->size_hints.win_gravity,
            &new_rect);
  }
  meta_window_constrain (window, window->frame ? &fgeom : ((MetaFrameGeometry*) 0), flags,
